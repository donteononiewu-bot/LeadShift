...
create type system_log_severity as enum ('info', 'warning', 'error');

create table if not exists system_logs (
  id uuid primary key default gen_random_uuid(),
  org_id uuid not null references organizations(id) on delete cascade,
  actor_id uuid references users(id) on delete set null, -- null = system/automation
...
create index if not exists system_logs_org_created_idx
  on system_logs(org_id, created_at desc);
create index if not exists system_logs_org_entity_idx
  on system_logs(org_id, entity_type, entity_id);
