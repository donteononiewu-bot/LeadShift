...
create table if not exists routing_rules (
  ...
  sources lead_source_type[] not null default '{}', -- empty = any source
...
alter table routing_events
  add constraint routing_events_routing_rule_id_fkey
  foreign key (routing_rule_id) references routing_rules(id) on delete set null;

-- Buyers eligible for a rule (the candidate pool the strategy picks among).
